...
#ifdef URHO3D_VULKAN
    if (gapi == GAPI_VULKAN)
        return SetDataRange_Vulkan(data, start, count, discard);
#endif
...
#ifdef URHO3D_VULKAN
    if (gapi == GAPI_VULKAN)
        return MapBuffer_Vulkan(start, count, discard);
#endif
